...
UDPSocket::UDPSocket(const std::string &localIP, const int localPort) {
  ...
  int y = 1;
  if (setsockopt(socket_, SOL_SOCKET, SO_REUSEADDR, (char *)&y, sizeof(y)) ==
      -1) {
    close(socket_);
    throw SocketException(strerror(errno));
  }
...
bool UDPSocket::setReceiveTimeout(const int seconds, const int micros) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,
                       sizeof(timeout));
...
bool UDPSocket::joinMulticastGroup(const std::string &ifIP,
  ...
  // See
  // https://sites.ualberta.ca/dept/chemeng/AIX-43/share/man/info/C/a_doc_lib/aixprggd/progcomc/skt_multicast.htm
  //    "If no interface is specified then the interface leading to the default
  //    route is used"

  // join multicast group
  int ret = setsockopt(socket_, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&mreq,
                       sizeof(mreq));
...
bool UDPSocket::receive(char *buf, size_t buflen) {
  ...
  ssize_t r =
      recvfrom(socket_, buf, buflen, 0, (sockaddr *)&remoteAddr, &addrlen);
...
bool UDPSocket::send(const std::string &remoteIP, const int remotePort,
  ...
  ssize_t r =
      sendto(socket_, buf, buflen, 0, (sockaddr *)&hostAddr, sizeof(hostAddr));
