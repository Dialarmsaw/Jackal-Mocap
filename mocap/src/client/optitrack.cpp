...
OptiTrack::OptiTrack(const Parameters &params) : params_(params) {}
...
bool OptiTrack::init() {
  ...
  client_ = std::make_unique<OptiTrackClient>(
      params_.localIP, params_.serverIP, params_.multicastIP,
      params_.commandPort, params_.dataPort);
...
std::vector<RigidBodyMeasurement> OptiTrack::getRigidBodyMeasurements() {
  ...
  const std::vector<Packet> pkts = client_->getPackets();
...
std::vector<Marker> OptiTrack::getUnlabeledMarkers() { return {}; }

// ----------------------------------------------------------------------------

std::vector<Camera> OptiTrack::getCameras() { return {}; }
...
double OptiTrack::getTotalLatency() {
  return (last_tx_timestamp_ns_ - last_camera_mid_exposure_timestamp_ns_) *
         1e-9;
}
...
double OptiTrack::getEstimatedTimeNsOffset() {
  ...
  const int64_t offset =
      last_rx_timestamp_ns_ - comm_delay - last_tx_timestamp_ns_;
