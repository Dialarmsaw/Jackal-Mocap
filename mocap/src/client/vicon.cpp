...
VICON::VICON(const Parameters &params) : params_(params) {}

// ----------------------------------------------------------------------------

VICON::~VICON() { client_.Disconnect(); }
...
bool VICON::init() {
  ...
  version << "VICON DataStream SDK v" << data.Major << "." << data.Minor << "."
          << data.Point;
  ...
  client_.SetStreamMode(
      ViconSDK::StreamMode::ServerPush); // this is the lowest latency version
  ...
  // Skip some frames to avoid the "leftover" subject names from last
  // conneciton. This is perhaps due to the ServerPush mode.
  for (int i = 0; i < 25; i++) {
...
uint64_t VICON::getCurrentTimestampNs() {
  ...
  uint64_t time_ns =
      (time.Seconds + 60 * time.Minutes + 3600 * time.Hours) * 1e9;

  // subsecond resolution
  double frames =
      time.Frames + static_cast<double>(time.SubFrame) / time.SubFramesPerFrame;
...
double VICON::getEstimatedTimeNsOffset() {
  // See diagram at
  // https://gitlab.com/mit-acl/fsw/mocap/-/issues/4#note_1164956152
  ...
  const double t_current_frame_rx_ns = t_current_frame_rx_ms * 1e6;
  ...
  // Assuming that the current frame timestamp is the time of camera
  // observation, then this quantity brings us to the time at which the data was
  // transmitted.
  const int64_t t_current_frame_tx_ns =
      (getCurrentTimestampNs() + t_capture_to_transmit_ns);

  const int64_t offset =
      t_current_frame_rx_ns - comm_delay - t_current_frame_tx_ns;
