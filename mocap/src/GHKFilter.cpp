...
GHKFilter::GHKFilter() {
  ...
  k_gain =
      0.5 * (1 - POSITION_THETA) * (1 - POSITION_THETA) * (1 - POSITION_THETA);
}
...
void GHKFilter::measUpdate(double dt, geometry_msgs::Point measure_pos) {
  if (not initialized_) {
    this->pos = measure_pos;
    initialized_ = true;
  } else {
    this->measUpdate_single(dt, measure_pos.x, this->pos.x, this->vel.x,
                            this->acc.x);
    this->measUpdate_single(dt, measure_pos.y, this->pos.y, this->vel.y,
                            this->acc.y);
    this->measUpdate_single(dt, measure_pos.z, this->pos.z, this->vel.z,
                            this->acc.z);
  }
}

void GHKFilter::propUpdate_single(double dt, double &xk, double &vk,
                                  double &ak) {

  // Propogation step
  double xkp =
      xk + dt * vk + 1. / 2. * dt * dt * ak; // update estimated position
