  ...
  try {
    ...
    for (const auto &vptr : camarray) {
      ...
      if (camera.enabled) {
        ...
        camera.orientation = Eigen::Quaterniond(
            Eigen::Vector4d(frpcarray2eigen(camstruct["orientation"])));
        camera.image_error = FRPC::Double(camstruct["image_error"]).getValue();
        camera.focal_length =
            FRPC::Double(camstruct["focal_length"]).getValue();
...
std::vector<Marker> RPCClient::getModelMarkers(const std::string &modelname) {
  ...
  try {
    const FRPC::Array_t &markerarray =
        FRPC::Array(client(pool, "getModelMarkers", pool.String(modelname)));
