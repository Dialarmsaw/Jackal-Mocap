...
void AttitudeEKF::measUpdate(geometry_msgs::Quaternion q_meas) {
  if (not initialized_) {
    xhatp(6) = q_meas.w;
    xhatp(3) = q_meas.x;
    xhatp(4) = q_meas.y;
...
geometry_msgs::Quaternion AttitudeEKF::getAtt() {
...
geometry_msgs::Vector3 AttitudeEKF::getRate() {
...
std::valarray<double> AttitudeEKF::vec(Eigen::MatrixXd m) {
  ...
  if (m.cols() != m.rows()) {
    ROS_ERROR_STREAM("Matrix in vec command is not square!");
  ...
  std::valarray<double> v(n * (n + 1) /
                          2); // relies on fact that sum 1 to n = n*(n+1)/2
...
Eigen::MatrixXd AttitudeEKF::unvec(std::valarray<double> v) {
  ...
  // lenV = n*(n+1)/2 from
  // http://en.wikipedia.org/wiki/1_%2B_2_%2B_3_%2B_4_%2B_%E2%8B%AF So, n^2 + n
  // - 2*lenV = 0 -> solve for largest root
...
unsigned int AttitudeEKF::largestRoot(unsigned int lenV) {
  ...
  if (d < 0) {
    ROS_ERROR_STREAM("Error in solving for largest root -- imaginary roots");
    return 0;
  }
  // return max root
  else if (d == 0) {
...
AttitudeEKF::processModel(Eigen::Matrix<double, ATT_STATES, 1> x) {
...
AttitudeEKF::transitionMatrix(Eigen::Matrix<double, ATT_STATES, 1> x) {
  double tau_rx = RATE_TIME_CONSTANT;
  double tau_ry = RATE_TIME_CONSTANT;
  double tau_rz = RATE_TIME_CONSTANT;
  ...
  Eigen::Matrix<double, ATT_STATES, ATT_STATES> phi =
      Eigen::MatrixXd::Zero(ATT_STATES, ATT_STATES);
