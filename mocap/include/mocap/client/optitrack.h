...
public:
  struct Parameters {
    std::string
        localIP; ///< Local (host) IP address for commands (e.g., 0.0.0.0)
    std::string serverIP; ///< IP address of OptiTrack computer
    std::string
        multicastIP;      ///< Multicast group IP (for NatNet rigid body data)
    int commandPort;      ///< Port number (server side?)
    int dataPort;         ///< Multicast data port number (for rigid body data,
                          ///< client/server side)
  };
...
private:
  ...
  std::unique_ptr<OptiTrackClient> client_; ///< OptiTrack NatNet client

  uint64_t last_server_time_ns_ =
      0; ///< (last) server time since Motive software start, [ns]
  uint64_t last_tx_timestamp_ns_ =
      0; ///< (last) transmit time of rigid body, [ns] (measured there)
  uint64_t last_rx_timestamp_ns_ =
      0; ///< (last) received time of rigid body, [ns] (measured here)
  uint64_t last_camera_mid_exposure_timestamp_ns_ =
      0; ///< (last) pose estimation time, [ns]

  MovingAverage<double, 300>
      offset_ns_ma_; ///< moving average estimator for time offset, [ns]
};
